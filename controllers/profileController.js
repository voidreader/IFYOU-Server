...
export const userProfileSave = async (req, res) =>{
    ...
    if(currencyList){
        ...
        for(const item of currencyList){
            result = await DB(`SELECT currency_type FROM com_currency WHERE currency = ?;`, [item.currency]);
            if(result.row[0].currency_type === "portrait" || result.row[0].currency_type === "frame" ) validCheck = false; // 초상화나 테두리가 있으면 false 
        }
