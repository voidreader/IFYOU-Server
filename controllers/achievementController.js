    ...
    if(result.state && result.row.length > 0){
        ...
        if(result.state){
            if(result.row.length === 0){
                resultQuery = mysql.format(`INSERT INTO user_achievement(userkey, achievement_id, gain_point, is_clear, clear_date) VALUES(?, ?, ?, 1, now());`, [userkey, achievement_id, gain_point]);
    ...
    if(achievement_id === 3){ //비기너
        ...
        if(result.state){
            ...
            for(const item of result.row){
                ...
                result = await DB(`SELECT * FROM user_achievement WHERE userkey = ? AND achievement_id = ? AND achievement_level = ?;`, [userkey, achievement_id, item.achievement_level]);
                if(result.state){
                    if(result.row.length === 0){
                        ...
                        INSERT INTO user_achievement(
                            ...
                            , now()
                            , ?
                        );`, [userkey, achievement_id, item.gain_point, item.achievement_level, totalCount]);
                    }else{
                        resultQuery += mysql.format(`
...
const checkFirstLiveIllust = async (userkey, achievement_id) => {
    ...
    //라이브 일러스트 개수 확인 
    result = await DB(`SELECT * FROM user_illust WHERE userkey = ? AND illust_type = 'live2d';`, [userkey]);
    if(result.state) totalCount = result.row.length; 
    ...
    if(totalCount >= achievement_point){
        ...
        if(result.state){
            if(result.row.length === 0){
                ...
                , current_result = ?
                WHERE userkey = ? 
                AND achievement_id = ?;`, [gain_point, totalCount, userkey, achievement_id]);
            }
        }
    }
...
const checkPurchaseSelection = async (userkey, achievement_id) =>{
    ...
    //과금 선택지 구매 건수 확인
    result = await DB(`SELECT * FROM user_selection_purchase WHERE userkey = ?;`, [userkey]);
    if(result.state) totalCount = result.row.length;

    if(achievement_id === 5){ //비기너
...
const checkWaitingCoin = async (userkey, achievement_id) => {
    ...
    AND CAST(JSON_EXTRACT(log_data, '$.price') AS UNSIGNED integer) <> 0;`, [userkey]);
    if(result.state) totalCount = result.row.length;

    if(achievement_id === 6){ //비기너
...
const checkAllClear = async (userkey, achievement_id, project_id) => {
    ...
    if(result.state){
        if(result.row.length === 0){
            resultQuery += mysql.format(`INSERT INTO user_achievement(userkey, achievement_id, gain_point, current_result, is_clear, clear_date) VALUES(?, ?, ?, 1, now());`, [userkey, achievement_id, gain_point, project_id]);
        }else{
            resultQuery += mysql.format(`UPDATE user_achievement SET gain_point = ?, current_result = ?, is_clear = 1, clear_date = now() WHERE userkey = ? AND achievement_id = ?;`, [gain_point, project_id, userkey, achievement_id]);
        }    
    }      
...
const checkHiddenEndingCount = async (userkey, achievement_id) => {
    ...
    WHERE userkey = ? 
    AND episode_id IN (SELECT episode_id FROM list_episode WHERE episode_type = 'ending' AND ending_type = 'hidden');`, [userkey]);
    if(result.state) totalCount = result.row.length;
...
const checkCurrencyUse = async (userkey, achievement_id) => {
    ...
    //소모량 확인
    result = await DB(`SELECT ifnull(sum(quantity), 0) total FROM gamelog.log_property WHERE userkey = ? AND log_type = 'use' AND currency = ?;`, [userkey, currency]);
            ...
            if(result.state){
                if(result.row.length === 0){
                    ...
                    INSERT INTO user_achievement(
                        ...
                        , now()
                        , ?
                    );`, [userkey, achievement_id, item.gain_point, item.achievement_level, episode_id]);
                }else{
                    resultQuery += mysql.format(`
                    UPDATE user_achievement 
                    SET gain_point = ?
    ...
    if(totalCount === achievement_point){
        ...
        if(result.state){
            if(result.row.length === 0)
                ...
                WHERE userkey = ? 
                AND achievement_id = ?
                AND is_clear = 0;`, [gain_point, project_id, userkey, achievement_id]); 
        }
    }
...
const checkReset = async (userkey, achievement_id, episode_id) =>{
    ...
    if(achievement_id === 16){ //처음부터 리셋 
        ...
        if(result.state){
            if(result.row.length === 0){
                resultQuery = mysql.format(`INSERT INTO user_achievement(userkey, achievement_id, gain_point, current_result, is_clear, clear_date) VALUES(?, ?, ?, 1, now());`, [userkey, achievement_id, gain_point, episode_id]);
            }else{
                resultQuery = mysql.format(`UPDATE user_achievement SET gain_point = ?, current_result = ?, is_clear = 1, clear_date = now() WHERE userkey = ? AND achievement_id = ?;`, [gain_point, episode_id, userkey, achievement_id]);
            }    
        }           
        ...
        FROM gamelog.log_action 
        WHERE userkey = ? 
        AND action_type ='reset_progress';`, [userkey]);
        if(result.state) totalCount = result.row.length;
...
export const checkStanding = async (userkey, achievement_id) => {
    ...
    if(result.state){
        if(result.row.length === 0)
            ...
            WHERE userkey = ? 
            AND achievement_id = ?
            AND is_clear = 0;`, [gain_point, totalCount, userkey, achievement_id]); 
    }
...
export const requestAchievementMain = async(req, res) =>{
    ...
    if(achievement_id === 1 || achievement_id === 2 || achievement_id === 3 || achievement_id === 4 || achievement_id === 5 || achievement_id === 6 ){
    ...
    if(achievement_id === 1){ //계정 연동          
        ...
        if(validCheck) query = await checkPurchaseSelection(userkey, achievement_id);
