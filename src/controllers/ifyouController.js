...
const getDailyMissionList = async (userkey, lang) =>{
    ...
    ON userkey = ? 
    AND now() BETWEEN udm.create_date AND concat(DATE_FORMAT(now(), '%Y-%m-%d'), ' 23:59:59')
...
export const requestDailyMissionReward = async(req, res) =>{
    ...
    AND is_active > 0;`, [userkey, mission_no]);
...
export const increaseDailyMissionCount = async (req, res) =>{
    ...
    if(mission_done === 1){
        ...
        result = await DB(`CALL pier.sp_update_user_daily_mission(?, ?);`, [userkey, mission_no]);
        if(!result.state){
